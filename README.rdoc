...
You will see that icon files, a javascript file and a stylesheet have been copied to +public+, and the configuration file 
<tt>wice_grid_config.rb</tt> is copied to <tt>config/initializers/</tt>.
...
The trick with the method returning a two element array (see section 'Custom dropdown filters') applies here as well, 
only for the last method in the queue, logically.
...
Use constant <tt>DYNARCH_CALENDAR_STYLE</tt> in <tt>config/initializers/wice_grid_config.rb</tt> to change the style of the calendar.
Use constant <tt>DYNARCH_CALENDAR_LANG</tt> in <tt>config/initializers/wice_grid_config.rb</tt> to change the language of the calendar.
