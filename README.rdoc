...
added which can containt the numeric range (more than, less than).
...
where the block contains just a call to the same attribute declared by :attribute_name, the block can be ommited:
...
It is possible to replace the grid with some alternative view if no filters are active  and there are no records to render:
