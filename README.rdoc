...
Version:: 0.4.1
Author::  Yuri Leikind
Website:: http://leikind.org/pages/wicegrid
...
The +initialize_grid+ method supports a <tt>:conditions</tt> parameter which is passed on to the underlying ActiveRecord (via will_paginate), but  it can only be in a string or an array format, the hash format is not supported at the moment:
...
If it is needed to retrieve the objects of the current selection that the user has selected with filters, for use outside of the grid,
WiceGrid offers two options.


==== Accessing selected records from the view

Use method +selected_records+ of a WiceGrid instance:
  ...
  <% if params[:process_selected_tasks] 
    ...
    <p><%= selected.size %> selected: <%= selected.map(&:id).to_sentence %></p>
  <% end -%>

This call can only be used after the view helper.

==== Accessing selected records from the controller

Because the real database work is done from the view helper, to access the selected records it is required to set up a callback to a controller
method. +initialize_grid+ accepts parameter <tt>:after</tt> to setup a callback to a method or a Proc object which will be called
with a parameter giving access to the selection of objects.


The code to implement this use case might look in the following way:

  def index
    @tasks_grid = initialize_grid(Task, :after => :process_selection)
  end

  def process_selection(wrapper_lambda)
    if params[:process_selected_tasks]
      @selected_tasks = wrapper_lambda.call
      # Do something with @selected_tasks
    end
  end

The object supplied to the callback is not the list of the selected objects but a Proc object that needs to be called to obtain this list.
This is done so to avoid redundant SQL calls each time when the page with the grid is rendered;
the list of objects is  <i>all pages</i> of the current filtered selection, and can be quite large.
